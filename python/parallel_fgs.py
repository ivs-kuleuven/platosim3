# # version of the script to run parallel simulations
...
outputDir   = os.pardir + "/Simulations" + "/newTest"
outputFilePrefix = "/newPsfTest"
...
for x in range(0, NguideStars, cores):
    ...
    if y > NguideStars:
        p.map(startSimulation, range(x, y-(y-NguideStars+1)))
    else:    
        p.map(startSimulation, range(x, y))
