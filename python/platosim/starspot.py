...
filename = os.getenv('PLATO_PROJECT_HOME') + '/inputfiles/data_varsim/varsim_meunier19a_t1.txt'
...
def simulate_lc(teff, time, dur, cadence_hours,
                incl = None, isim = 0, odir = None,
                verbose = False, doplot = False, save = False):
    if odir is None:
        odir = os.getcwd()
    # select parameters
    ...
    if verbose:
        print('T_eff         : {:.0f} K'.format(teff))
        print('B-V           : {:.3f} mag'.format(bv))
        print("log R'_HK     : {:.3f}".format(lrhk))
        print('P_rot         : {:.3f} days'.format(prot))
        print('P_min         : {:.3f} days'.format(pmin))
        print('P_max         : {:.3f} days'.format(pmax))
        print('max. latitude : {:.3f} deg'.format(lmax))
        print('P_cycle       : {:.3f} years'.format(clen))
    ...
    if doplot and dF is not None:
        fig, axes = plt.subplots(3,1, figsize=(12,7), sharex=True)
        ttl= 'Activity model: AR={:5.3f} CL={:6.3f}, i={:6.2f}, Pmin={:6.2f}, Pmax={:6.2f}, Lmax={:5.2f}'.format(arate, clen,
                                                                                                                 incl,
                                                                                                                 pmin, pmax, lmax)
        ...
        for j in range(s.nspot):
            ...
            if s.t0[j] > dur:
                continue
            axes[0].plot(s.t0[j], s.lat[j], 'ko', markersize = s.amax[j]*(1./3e-4)*5, alpha = 0.8)
        axes[0].set_ylim(-90,90)
        axes[0].set_ylabel('Spot lat. [deg]')
        axes[1].plot(time, area.sum(0)*100, 'k-')
        axes[1].set_ylabel(r'Spot coverage [\%]')        
        axes[2].plot(time, dF.sum(0)*1e6, 'k-')
    ...
    # Finito!
    params = np.array([bv, lrhk, prot, pmin, pmax, lmax, clen, coverlap, arate, incl], dtype=np.float32)
    return flux, params 


if __name__ == '__main__':
    simulate_lc(teff, time, dur, cadence_hours, incl=90.0, isim=0,
                verbose=False, doplot=False, save=False)
