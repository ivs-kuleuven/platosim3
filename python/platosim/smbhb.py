...
def modelRedNoisePSD(freq, timescale, varscale):
    ...
    OUTPUT: psd:  power spectral density    | ppm^2/microHz | ppm^2/microHz]
    """
    psd = np.zeros_like(freq)
    for n in range(len(timescale)):
        sigma = varscale[n]
        tau = timescale[n]
        psd += tau**2 * sigma**2 / (1 + (2.0*np.pi*freq*tau)**2)
    return psd
...
class model(object):
    ...
    def light_curve(self, time, df=False):
        ...
        if self.tau is not None:
            ...
            flux_red = modelRedNoise(time, tau, sigma, seed=self.seed) * 1e-6 + 1
            flux += (flux_red - 1)
            if df:
                return get_df(time, flux, flux_boost, flux_lens, flux_red)
            else:
                return flux, flux_boost, flux_lens, flux_red
        else:
            if df:
                get_df(time, flux, flux_boost, flux_lens)
            else:
                return flux, flux_boost, flux_lens
...
def plot_result(df, result, z, tdur=3, dt=0.1, alpha=0.5, figsize=(9,10)):
    ...
    ax[0].errorbar(df.time, df.flux, yerr=df.flux_err, fmt='.k', alpha=alpha, zorder=1)
    ax[0].plot(dm.time, dm.flux, '-', c='royalblue')
    ax[1].errorbar(df.time, residuals, yerr=df.flux_err, fmt='.k', alpha=alpha, zorder=1)
    ax[1].plot(dm.time, np.zeros_like(dm.time), '--', c='orange')
