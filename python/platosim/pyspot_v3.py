...
######### STILL TBD #########################
# get limb-darkening coef from teff         #
# include faculae (with area, contrast      #
# and limb-brightening coef from teff too)  #
#############################################
...
ASUN = 0.12 # in log R'_HK, based on solar HARPS-N observations to date
PROT_SUN = 27.0
...
def get_acyc_from_bv_and_lrhk(bv, lrhk, use_max = False):
    if bv < 0.851:
...
class spots():
    ...
    def __init__(self, reg_arr,
        ...
        self.lat = lat[l]
        self.lon = lon[l]
        self.mu = np.zeros_like(self.lon) + mu # linear limb darkening
        ...
        self.decay_rate = rng.lognormal(mean=lmed, sigma=sig, size=self.nspot)
...
#         area[l] = self.amax[i] * (1-(time[l]-self.t0[i]) / decay_time)
        ...
        beta = np.cos(self.incl * DEG2RAD) * np.sin(self.lat[i] * DEG2RAD) + np.sin(self.incl * DEG2RAD) * np.cos(self.lat[i] * DEG2RAD) * np.cos(phase)
        # Differential effect on stellar flux
        dF = dorren_F(self.mu[i], self.mu[i], 0.0, np.arcsin(np.sqrt(area)), np.arccos(beta))
        # if self.mu[i] == 0:
        #     dF = - area * beta
        #     dF[beta < 0] = 0
        # else:
        #     dF = dorren_F(self.mu[i], self.mu[i], 0.0, np.arcsin(np.sqrt(area)), beta)
