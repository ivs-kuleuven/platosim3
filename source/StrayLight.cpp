...
void StrayLight::configure(ConfigurationParameters &configParam)
{
  ...
  std::vector<arma::vec> celestialObjectSpectralRadiance =
      getCelestialObjectGridSpectralRadiance(sun[0], moon[0], moon_reflectivity,
                                             grid);

  getIrradianceAtCamera(camera, grid, celestialObjectSpectralRadiance, moon[0],
                        sc[0]);
  ...
  for (unsigned int i = 0; i < 29; i++)
  {
    ...
    double radiance =
        ...
        (std::pow(wavelength, 5) *
         (std::pow(Constants::E,
                   (Constants::HPLANCK * Constants::CLIGHT /
                    (wavelength * Constants::KBOLTZMANN * Constants::SOLART))) -
          1));
...
void StrayLight::getIrradianceAtCamera(
    Camera &camera, std::vector<GridPoint> grid,
    std::vector<arma::vec> emmitterIrradiance, arma::vec emmitterPosition,
    arma::vec cameraPosition)
{
  ...
  for (unsigned int idx = 0; idx < grid.size(); idx++)
  {
    ...
    arma::vec n_Emmitter_Cam =
        emmitterPosition + grid[idx].point - cameraPosition;
    n_Emmitter_Cam = n_Emmitter_Cam /
                     sqrt(arma::as_scalar(n_Emmitter_Cam.t() * n_Emmitter_Cam));

    double cos_alpha1 =
        arma::as_scalar(grid[idx].point.t() * n_Emmitter_Cam) /
