 ...
 * \param telescope     Telescope on which the camera is mounted on
 * \param sky           Sky object to query which stars we are seeing
 * \param psf           PointSpreadFunction to estimate distortion coeficients
 *
 * \note This constructor is only used when we have PSF that is mapped from file. The
 *        other constructor does not initialize the psf variable.
 */

Camera::Camera(ConfigurationParameters &configParam, HDF5File &hdf5file, Platform &platform, Telescope &telescope, Sky &sky, PointSpreadFunction &psf)
  : HDF5Writer(hdf5file), platform(platform), telescope(telescope), sky(sky), focalLength(nullptr), focalPlaneAngle(nullptr),
    distortionCoef(nullptr), inverseDistortionCoef(nullptr), internalTime(0.0), includeFieldDistortion(true), userGivenSkyBackground(-1.0),
    fluxOfV0Star(0.0), psf(&psf)
{
    // Parse the parameters from the configuration file.
...
void Camera::flushOutput()
{
    ...
    if ((includePointLikeGhosts || includeExtendedGhosts) && writeGhostPositions)
    {
        if(includePointLikeGhosts)
        {
            if (groupByExposure)
            {
                hdf5File.writePointlikeGhostByExposure(detectedPointLikeGhostInfo, beginExposureNr);
            }
            else
            {
                hdf5File.writePointlikeGhostByStarID(detectedPointLikeGhostInfo);
            }
        }
        ...
        if(includeExtendedGhosts)
        {
            if (groupByExposure)
            {
            hdf5File.writeExtendedGhostByExposure(detectedExtendedGhostInfo, beginExposureNr);
            }
            else
            {
            hdf5File.writeExtendedGhostByStarID(detectedExtendedGhostInfo);
            }
        }
    }
   ...
   // Idem for the inverse distortion coefficients.
    ...
    if (includeFieldDistortion || isMapped)
    {
        ...
        else
        {
            ...
            vector<double> inverseDistortionCoefVector;

            if (isMapped)
            {
                distortionCoefVector = psf->estimateDistortionCoefficients((*focalLength)());
                inverseDistortionCoefVector = psf->estimateInverseDistortionCoefficients((*focalLength)());
            }
            else if (fieldDistortionSource == "ConstantValue")
            {
                ...
                inverseDistortionCoefVector = configParam.getDoubleVector("Camera/FieldDistortion/ConstantInverseCoefficients");
            }
...
void Camera::exposeDetectorWithStars(Detector &detector, double startTime, double exposureTime, double readoutTimeBeforeNextExposure)
{
    ...
    while (internalTime < startTime + exposureTime)
    {
	    ...
	    if (find(starIDsInSubfield.begin(), starIDsInSubfield.end(), starID) == starIDsInSubfield.end())
            {
	        starIDsInSubfield.push_back(starID);
            }
