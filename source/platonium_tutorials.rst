...
**SLURM setup:** If you want to run your simulation in parallel on a computing cluster using SLURM, add the flag ``--slurm`` and the following two files are saved to your output directory:
  
  - ``cluster.slurm``     : A SLURM job script to run parallel computations
  - ``cluster_<CAM>.data``: A SLURM parameterisation file
...
HereWhile using the ``--project`` output path, the files will be generated directly into your working directory and will immediately be known and used when running ``platonium`` later.
   



    
Beyond the input files that are possible to parse to PlatoSim (see `here <http://ivs-kuleuven.github.io/PlatoSim3/_input_file_description.html>`_), the first three files listed above are so-called Pointing Error Sources (PES) files. I.e. systematic noise sources that directly impact the source PSF either in position, shape, or both. The output format of these two files are directly in an input format that PlatoSim can interpret:
