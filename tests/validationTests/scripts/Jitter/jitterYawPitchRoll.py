...
"""This test is designed to check weather the jitter behaves as RMS * N(0, 1), where RMS is the standard deviation parameter for the jitter. We do this by checking that the standard deviation and the mean of the drift scale linearly with input RMS. The RMS parameters are given from the input file."""
...
class JitterYawPitchRoll(Test):
    ...
    def runSimulation(self):
        ...
        # We run the simulation three times, each time we allow jitter in one dimension of rotation
        # while the other angles are kept constant.

        self.axes = ["JitterYawRms", "JitterPitchRms", "JitterRollRms"]

        self.resultMean = []
        self.resultSdf  = []

        # The simulation is then run for multiple RMS values, and we save the (mean jitter) / RMS
        # and (standard deviation of jitter) / RMS for every run. We would expect this value to be
    ...
    def runForRMS(self, rms, ax):
        ...
        # The simulation is run for a given jitter RMS around one given axis = ax.
        # The jitter for every exposure is saved in the variable delta and the function returns
        # the mean / RMS and (standard deviation) / RMS of the jitter for all the exposures.

        numEx = self.sim["ObservingParameters/NumExposures"]

        self.sim["Platform/" + self.axes[ax]]   = rms
        simFile = self.sim.run(removeOutputFile = True)
        self.sim["Platform/" + self.axes[ax]] = 0
